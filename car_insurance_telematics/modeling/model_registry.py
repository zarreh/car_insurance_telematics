...
class ModelRegistry:
    ...
    def save_model(
        self, model: Any, name: str, model_type: str, metrics: Dict[str, Any], feature_names: List[str]
    ) -> str:
        """
        ...
        Args:
            ...
            name: Name of the model ('claim_probability' or 'claim_severity')
            model_type: Type of algorithm (e.g., 'random_forest', 'gradient_boosting')
        ...
        existing_versions = [v for v in self.registry["models"][name].keys() if v != "latest"]
        ...
        logger.info(f"Model saved: {name} {version} ({model_type}) at {model_path}")
