...
class FeatureEngineer:
    ...
    def _create_basic_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        features['trip_duration_minutes'] = df['trip_duration_minutes'] if 'trip_duration_minutes' in df.columns else 0
        features['trip_distance_km'] = df['trip_distance_km'] if 'trip_distance_km' in df.columns else 0
        ...
        features['trip_efficiency'] = np.where(
            features['trip_distance_km'] > 0,
            features['trip_duration_minutes'] / features['trip_distance_km'],
            0
        )
    ...
    def _create_speed_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Basic speed metrics
        features['max_speed_kmh'] = df['max_speed_kmh'] if 'max_speed_kmh' in df.columns else 0
        features['avg_speed_kmh'] = df['avg_speed_kmh'] if 'avg_speed_kmh' in df.columns else 0
        ...
        features['speed_consistency'] = np.where(
            features['max_speed_kmh'] > 0,
            features['avg_speed_kmh'] / features['max_speed_kmh'],
            0
        )
    ...
    def _create_behavior_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        features['hard_braking_count'] = df['hard_braking_count'] if 'hard_braking_count' in df.columns else 0
        features['hard_acceleration_count'] = df['hard_acceleration_count'] if 'hard_acceleration_count' in df.columns else 0
        features['sharp_turn_count'] = df['sharp_turn_count'] if 'sharp_turn_count' in df.columns else 0

        # Total aggressive events
        features['total_aggressive_events'] = (
            features['hard_braking_count'] + 
            features['hard_acceleration_count'] + 
            features['sharp_turn_count']
        )

        # Aggressive driving rates (per km)
        distance = df['trip_distance_km'] if 'trip_distance_km' in df.columns else pd.Series([1] * len(df))
        distance = pd.Series(distance).clip(lower=0.1)  # Avoid division by zero
        ...
        duration_minutes = df['trip_duration_minutes'] if 'trip_duration_minutes' in df.columns else pd.Series([1] * len(df))
        duration_hours = pd.Series(duration_minutes / 60).clip(lower=0.01)
    ...
    def _create_time_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        features['night_driving_minutes'] = df['night_driving_minutes'] if 'night_driving_minutes' in df.columns else 0
        features['rush_hour_minutes'] = df['rush_hour_minutes'] if 'rush_hour_minutes' in df.columns else 0

        # Proportions
        total_duration = df['trip_duration_minutes'] if 'trip_duration_minutes' in df.columns else pd.Series([1] * len(df))
        total_duration = pd.Series(total_duration).clip(lower=1)
        ...
        # Weekend indicator
        features['weekend_driving'] = df['weekend_driving'] if 'weekend_driving' in df.columns else 0
    ...
    def _create_risk_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Phone usage
        features['phone_use_minutes'] = df['phone_use_minutes'] if 'phone_use_minutes' in df.columns else 0
        total_duration = df['trip_duration_minutes'] if 'trip_duration_minutes' in df.columns else pd.Series([1] * len(df))
        total_duration = pd.Series(total_duration).clip(lower=1)
        features['phone_use_ratio'] = features['phone_use_minutes'] / total_duration
        ...
        # Fatigue indicators
        features['continuous_driving_hours'] = df['continuous_driving_hours'] if 'continuous_driving_hours' in df.columns else 0
        ...
        features['adverse_weather'] = df['adverse_weather'] if 'adverse_weather' in df.columns else 0
        features['rain_driving'] = df['rain_driving'] if 'rain_driving' in df.columns else 0
        features['snow_driving'] = df['snow_driving'] if 'snow_driving' in df.columns else 0
        features['fog_driving'] = df['fog_driving'] if 'fog_driving' in df.columns else 0
        ...
        # Traffic density
        features['high_traffic_ratio'] = df['high_traffic_ratio'] if 'high_traffic_ratio' in df.columns else 0
    ...
    def _create_interaction_features(self, features_df: pd.DataFrame) -> pd.DataFrame:
        ...
        features['speed_aggression_interaction'] = (
            features_df.get('max_speed_kmh', 0) * 
            features_df.get('total_aggressive_events', 0) / 100
        )

        # Night driving and speed interaction
        features['night_speed_risk'] = (
            features_df.get('night_driving_ratio', 0) * 
            features_df.get('max_speed_kmh', 0) / 100
        )

        # Distance and aggressive events interaction
        features['long_trip_aggression'] = (
            features_df.get('log_trip_distance', 0) * 
            features_df.get('aggressive_events_rate', 0)
        )

        # Phone use and speed interaction
        features['distracted_speeding'] = (
            features_df.get('phone_use_ratio', 0) * 
            features_df.get('speeding_severity', 0)
        )

        # Fatigue and time of day interaction
        features['fatigue_night_risk'] = (
            features_df.get('continuous_driving_hours', 0) * 
            features_df.get('night_driving_ratio', 0)
        )

        # Weather and aggressive driving interaction
        features['weather_aggression_risk'] = (
            features_df.get('weather_risk_score', 0) * 
            features_df.get('total_aggressive_events', 0)
        )
    ...
    def validate_features(self, features_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if len(high_missing) > 0:
            validation_results['warnings'].append(
                f"High missing values in features: {list(high_missing.index)}"
            )
        ...
        if inf_features:
            validation_results['issues'].append(
                f"Infinite values in features: {inf_features}"
            )
        ...
        for col in features_df.select_dtypes(include=[np.number]).columns:
            ...
            if 'count' in col or 'minutes' in col or 'hours' in col:
                if col_min < 0:
                    validation_results['issues'].append(
                        f"Negative values in {col}: min={col_min}"
                    )
                    validation_results['valid'] = False
            ...
            if 'ratio' in col or 'indicator' in col:
                if col_min < 0 or col_max > 1:
                    validation_results['warnings'].append(
                        f"Values outside [0,1] in {col}: [{col_min}, {col_max}]"
                    )
