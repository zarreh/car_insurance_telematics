...
class FeatureEngineer:
    ...
    def _create_behavior_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        features["hard_acceleration_count"] = (
            df["hard_acceleration_count"] if "hard_acceleration_count" in df.columns else 0
        )
        ...
        duration_minutes = (
            df["trip_duration_minutes"] if "trip_duration_minutes" in df.columns else pd.Series([1] * len(df))
        )
    ...
    def _create_risk_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Fatigue indicators
        features["continuous_driving_hours"] = (
            df["continuous_driving_hours"] if "continuous_driving_hours" in df.columns else 0
        )
